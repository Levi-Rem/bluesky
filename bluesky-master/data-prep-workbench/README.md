...
独立子项目：为训练/仿真准备基础飞行数据（导航、机场、空域、航路、气象、机型性能、雷达与 ASTERIX 通道），提供 Excel 批量导入导出与全窗地图编辑器。作为与 `bluesky-master` 平级的独立 Maven 工程交付；其向训练平台提供运行参考数据的边界以[《模拟飞行员工作台第二版详细设计》](../docs/模拟飞行员工作台第二版详细设计.md)第 3.3、11 节为唯一实施依据。
