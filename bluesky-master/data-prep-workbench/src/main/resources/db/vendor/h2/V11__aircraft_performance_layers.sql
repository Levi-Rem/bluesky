...
SELECT id, id, 0, CONCAT('F', CAST(maximum_altitude_ft / 100 AS INT)),
       CONCAT('F', CAST(maximum_altitude_ft / 100 AS INT)),
       CAST(default_bank_angle_deg AS INT), CAST(default_bank_angle_deg AS INT),
       revision, deleted, created_by, updated_by
FROM aircraft_type_performance;
