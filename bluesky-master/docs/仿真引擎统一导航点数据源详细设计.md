...
| N-24 | 点指令 | 建机、直飞、指定点盘旋/等待、重新加入航线等均由训练平台解析点，再向引擎传递点对象。 |
...
| N-36 | 历史引用 | 重启后已有航空器或指令引用不存在点时保留记录用于审计，但标记无效并禁止运行，不自动删除或替换。 |
...
5. 将建机和所有点引用指令改为传递已解析点对象，不依赖 BlueSky 内置导航查询。
6. 将机场、航路点搜索从 Adapter 迁移至训练平台内存目录。
7. 将引擎连接状态与参考数据就绪状态独立建模，并建立运行门禁。
8. 对启动失败、校验失败、同步失败、断线重连和历史坏引用提供确定行为。
9. 补齐数据准备、训练平台、Adapter、前端与真实进程级联调测试。
...
- Adapter 首次连接或重连后自动重发同一内存数据，允许技术重试同一批数据。
- 重连同步不访问数据准备模块。
...
2. `DCT`、指定点 `ORBIT/HOLD`、接回航线及其他点引用指令携带 `targetPoint` 或等价强类型字段。
...
1. 启动后重新校验数据库中保留的航空器、未飞航路和待执行点指令。
2. 引用不存在或已排除点时保留原记录，记录稳定失败原因，并禁止开始或继续运行。
3. 不自动删除历史记录，不替换为最近点或同名其他类型点。
4. `WAYPOINT` 数量为 0 时参考数据未就绪。
5. `AIRWAY` 数量为 0 不阻断就绪；直接点航线仍可用。
6. 扇区或天气数量为 0 不影响参考数据就绪。
...
1. 启动 8090、Adapter、8080，记录数据准备端点恰好一次请求。
2. 验证地图航路点数量、训练平台目录数量和 Adapter 确认数量一致。
3. 使用含航路段的航线建机，核对 Adapter/BlueSky 中的实际点序列与坐标。
4. 验证 DCT 使用准备模块坐标；构造 BlueSky 内置同名异坐标点，证明不会被采用。
...
| 训练平台单元 | `RuntimeReferenceCatalogTest` | 一次加载、索引、状态、计数、空数据 |
| 训练平台单元 | `ReferenceServiceTest` | 本地搜索分类、排序、零 Adapter 调用 |
| 训练平台单元 | `RouteExpanderTest` | 语法、方向、歧义、去重、稳定错误码 |
| 训练平台协议 | `ZeroMqSimulationGatewayTest` | 全量同步和点对象命令，不含版本字段 |
| 训练平台服务 | `AircraftServiceTest`、`InstructionServiceTest` | 解析前置、原子失败、批量独立性 |
| 训练平台 API | `ReferenceDataReadinessApiTest` | bootstrap、门禁、历史坏引用 |
| Python Adapter | `test_reference_sync.py` | 全量校验、原子替换、数量确认 |
| Python Adapter | `test_resolved_points.py` | 建机/DCT/HOLD/ORBIT 使用传入坐标，不查内置导航库 |
| 前端 store/组件 | `store.test.ts`、`App.test.ts` | 独立状态、禁用、无弹窗、状态灯说明 |
| 跨模块集成 | 新 Spring 集成测试 | 启动一次请求、缓存地图、同步确认 |
| 进程级 E2E | PowerShell/Python 验证脚本 | 端点请求计数、重启、断线、同名异坐标证明 |
...
- **NAV-AC-24** BlueSky 内置导航数据库不参与建机、航线、DCT、ORBIT、HOLD 或接回航线的点解析。
...
- **NAV-AC-33** 历史记录引用不存在点时保留记录并阻断运行，不自动删除或替换。
...
- **NAV-AC-38** 至少保留一次真实 8090/8080/Adapter 三进程联调证据，包含请求计数、点数量一致性、同名异坐标验证和断线重连结果。

## 18. 完成定义

1. 第 17 节 38 条验收标准均有自动测试或可重复人工证据。
2. 新旧文档中不再存在“机场/航路点由 Adapter 查询 BlueSky 内置资源”的现行口径。
3. Java 与 Python 使用同一组协议测试向量验证点字段和数量确认。
